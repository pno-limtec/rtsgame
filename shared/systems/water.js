...
// sperren den Fluss, Quellen speisen Flüsse, Versickerung lässt Fluten mit der Zeit ablaufen.
//
...
const FLOW_DEPTH_EPS = WET_DEPTH * 0.9; // dünner Regenfilm = Bodenfeuchte, kein teurer Oberflächenfluss
...
function simulateCA(t, dtW, world) {
  ...
  // Senken/Gräben füllen sich, Flüsse schwellen. Nach dem Regen läuft das Wasser über die
  // Versickerung (nur ÜBER baseWater) wieder ab. Deterministisch über world.rng gesampelt.
  ...
  if (precip && !floodCap) {
    ...
    for (let k = 0; k < drops; k++) {
      ...
      water[i] = Math.min(WATER_MAX_DEPTH, water[i] + RAIN_DEPTH * mult);
      waterActive.add(i);
    }
    ...
    if (t.valleys) {
      const valleyGain = RAIN_DEPTH * mult * 4.8;
  ...
  if (t.snow && t.snowIdx && world && world.env) {
    if ((weather === 'clear' || drought) && solar > 0.25 && !floodCap) {
      ...
      for (const i of t.snowIdx) t.snow[i] = Math.min((height[i] - SNOW_LINE) * 10, t.snow[i] + snowGain);
    }
  }
  ...
  for (let k = 0; k < active.length; k++) {
    ...
    if (avail <= 0) continue;
    const si = flowGround(t, i) + water[i]; // eigene Oberfläche über gerilltem Boden
    // Tiefer liegende, nicht gesperrte Nachbarn sammeln.
    let lower = null, headSum = 0;
    const x = i % w, y = (i / w) | 0;
    for (const [dx, dy, dist] of DIR8) {
      const nx = x + dx, ny = y + dy;
      if (nx < 0 || ny < 0 || nx >= w || ny >= h) continue;
      ...
      const head = (si - (flowGround(t, j) + water[j])) / dist;
      if (head > FLOW_EPS) {
        const weight = head / dist;
    ...
    if (lower) {
      ...
      const mobility = Math.min(1, Math.max(0.18, avail / Math.max(WET_DEPTH * 1.4, 0.03)));
      const basinDamping = (t.lakeMask && t.lakeMask[i]) ? 0.18 : 1;
      const out = Math.min(avail, headSum * 0.60 * WATER_FLOW * mobility * basinDamping);
      if (out > SETTLE_EPS) {
  ...
  for (const [i, d] of delta) {
    ...
    if (Math.abs(d) > SETTLE_EPS && shouldSimulateWater(t, i)) markActive(t, next, i);
  }
  for (const i of touched) {
    ...
    if (t.startSafe && t.startSafe[i] && excess > SETTLE_EPS) {
      ...
      if (lake || (!thinMoisture && (drain > 0.0012 || hasSurfaceGradient(t, i, FLOW_EPS * 1.8)))) markActive(t, next, i);
...
function dryRiverBeds(t, amount, waterActive) {
  ...
  for (const path of t.riverPaths) for (const pi of path) {
    ...
    for (let dy = -1; dy <= 1; dy++) for (let dx = -1; dx <= 1; dx++) {
      const nx = x + dx, ny = y + dy;
      if (!inBounds(t, nx, ny) || Math.hypot(dx, dy) > 1.45) continue;
...
function applyFloodDamage(world, didStep) {
  ...
  for (const e of world.entities.values()) {
    ...
    if (depth > CURRENT_MIN_DEPTH) {
      ...
      if (cur.grad > 0) {
        const domainMult = e.domain === 'water' || e.domain === 'amphibious' ? 0.35 : (depth > FLOOD_DEPTH ? 1.35 : 1);
